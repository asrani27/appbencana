...
@section('page-subtitle', 'Kelola data rumah sakit')
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div class="flex items-center gap-3">
        <div class="relative flex-1 sm:flex-none">
            <form method="GET" action="{{ route('admin.rumahsakit.index') }}">
    ...
    <a href="{{ route('admin.rumahsakit.create') }}"
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-300 overflow-hidden">
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-blue-600 border-b border-blue-700">
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">No. HP</th>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Kapasitas IGD</th>
            ...
            <tbody class="divide-y divide-gray-300">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-4 text-center text-sm text-gray-600">
                        {{ ($rumahSakit->currentPage() - 1) * $rumahSakit->perPage() + $loop->iteration }}
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="font-medium text-gray-800">{{ $rs->nama }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <p class="text-sm text-gray-600 max-w-xs truncate">{{ Str::limit($rs->alamat, 50) }}</p>
                    </td>
                    <td class="px-6 py-4">
                        <p class="text-sm text-gray-600">{{ $rs->no_hp ?? '-' }}</p>
                    </td>
                    <td class="px-6 py-4">
                        <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-blue-100 text-blue-700">
                            {{ $rs->kapasitas_igd ?? 0 }} orang
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('admin.rumahsakit.show', $rs->id) }}"
                            ...
                            <a href="{{ route('admin.rumahsakit.edit', $rs->id) }}"
                            ...
                            <form action="{{ route('admin.rumahsakit.destroy', $rs->id) }}" method="POST"
                                onsubmit="return confirm('Yakin ingin menghapus data rumah sakit ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                ...
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <p class="text-gray-500 font-medium">Tidak ada data rumah sakit</p>
                            <p class="text-gray-400 text-sm mt-1">Tambahkan data rumah sakit baru untuk memulai</p>
                        </div>
                    </td>
                </tr>
                @endforelse
    ...
    <div class="px-6 py-4 border-t border-gray-300">
        {{ $rumahSakit->withQueryString()->links() }}
    </div>
    @endif
</div>
@endsection
