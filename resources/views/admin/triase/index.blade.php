...
@section('page-title', 'Triase')
@section('page-subtitle', 'Kelola data triase korban bencana')
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div class="flex items-center gap-3">
        <div class="relative flex-1 sm:flex-none">
            <form method="GET" action="{{ route('admin.triase.index') }}">
        ...
        <form method="GET" action="{{ route('admin.triase.index') }}" class="hidden sm:block">
    ...
    <a href="{{ route('admin.triase.create') }}"
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-blue-600 border-b border-blue-700">
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Korban
                    </th>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Bencana
                    </th>
                    <th class="px-6 py-4 text-center text-xs font-semibold text-white uppercase tracking-wider">Kategori
                    </th>
            ...
            <tbody class="divide-y divide-gray-300">
                @forelse($triases as $triase)
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-4 text-center text-sm text-gray-600">
                        {{ ($triases->currentPage() - 1) * $triases->perPage() + $loop->iteration }}
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            <div
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500">
                                    {{ $triase->korban->jenis_kelamin ?? '-' }}, {{ $triase->korban->umur ?? '-' }}
                                    tahun
                                </p>
                            </div>
                        </div>
                    ...
                    <td class="px-6 py-4">
                        <p class="text-sm text-gray-600">{{ $triase->korban->bencana->nama_bencana ?? 'N/A' }}</p>
                        <p class="text-xs text-gray-400">
                            {{ $triase->korban->bencana->tanggal ?? '-' }}
                        </p>
                    </td>
                    <td class="px-6 py-4 text-center">
                        @php
                        $kategoriClasses = [
                        'merah' => 'bg-red-100 text-red-700 border border-red-200',
                        'kuning' => 'bg-yellow-100 text-yellow-700 border border-yellow-200',
                        'hijau' => 'bg-green-100 text-green-700 border border-green-200',
                        ...
                        $kategoriClass = $kategoriClasses[$triase->kategori] ?? 'bg-gray-100 text-gray-700';
                        @endphp
                        <span
                            class="inline-flex items-center px-3 py-1 rounded-lg text-xs font-semibold {{ $kategoriClass }}">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('admin.triase.show', $triase->id) }}"
                            ...
                            <form action="{{ route('admin.triase.destroy', $triase->id) }}" method="POST"
                                onsubmit="return confirm('Yakin ingin menghapus data triase ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                ...
                <tr>
                    <td colspan="7" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <p class="text-gray-500 font-medium">Tidak ada data triase</p>
                            <p class="text-gray-400 text-sm mt-1">Tambahkan data triase baru untuk memulai</p>
                        </div>
                    </td>
                </tr>
                @endforelse
