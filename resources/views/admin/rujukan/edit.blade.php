...
@section('page-title', 'Edit Rujukan')
@section('page-subtitle', 'Perbarui data rujukan')
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-300 overflow-hidden">
    <div class="p-6 border-b border-gray-300">
        <h3 class="text-lg font-semibold text-gray-800">Form Edit Rujukan</h3>
        <p class="text-sm text-gray-500 mt-1">Lengkapi form di bawah untuk memperbarui data rujukan</p>
    </div>

    <form action="{{ route('admin.rujukan.update', $rujukan) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="korban_id" id="korban_id" required
                    ...
                    @foreach ($korban as $k)
                    <option value="{{ $k->id }}" {{ old('korban_id', $rujukan->korban_id) == $k->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select name="rumah_sakit_id" id="rumah_sakit_id" required
                    ...
                    <option value="{{ $rs->id }}" {{ old('rumah_sakit_id', $rujukan->rumah_sakit_id) == $rs->id ? 'selected' : '' }}>
                        {{ $rs->nama }}
                    </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="status" id="status" required
                    ...
                    <option value="dirujuk" {{ old('status', $rujukan->status)=='dirujuk' ? 'selected' : '' }}>Dirujuk</option>
                    <option value="diterima" {{ old('status', $rujukan->status)=='diterima' ? 'selected' : '' }}>Diterima</option>
                    <option value="selesai" {{ old('status', $rujukan->status)=='selesai' ? 'selected' : '' }}>Selesai</option>
                </select>
                @error('status')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="datetime-local" name="waktu_rujuk" id="waktu_rujuk" value="{{ old('waktu_rujuk', $rujukan->waktu_rujuk ? $rujukan->waktu_rujuk->format('Y-m-d\TH:i') : '') }}"
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="catatan" id="catatan" rows="4"
                    ...
                    placeholder="Catatan atau keterangan tambahan">{{ old('catatan', $rujukan->catatan) }}</textarea>
                @error('catatan')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mt-6 p-4 bg-gray-50 rounded-xl border border-gray-300">
            <h4 class="text-sm font-medium text-gray-700 mb-3">Informasi Rujukan</h4>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                    <p class="text-gray-500">ID Rujukan</p>
                    <p class="font-medium text-gray-800">#{{ $rujukan->id }}</p>
                </div>
                <div>
                    <p class="text-gray-500">Dibuat</p>
                    <p class="font-medium text-gray-800">
                        {{ $rujukan->created_at->locale('id')->translatedFormat('d M Y, H:i') }}</p>
                </div>
                <div>
                    <p class="text-gray-500">Terakhir Diperbarui</p>
                    <p class="font-medium text-gray-800">
                        {{ $rujukan->updated_at->locale('id')->translatedFormat('d M Y, H:i') }}</p>
                </div>
            </div>
        </div>
