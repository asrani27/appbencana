...
@section('page-subtitle', 'Kelola data rujukan korban')
    ...
    <div class="flex items-center gap-3">
        <div class="relative flex-1 sm:flex-none">
            <form method="GET" action="{{ route('admin.rujukan.index') }}">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari rujukan..."
        ...
        <form method="GET" action="{{ route('admin.rujukan.index') }}" class="hidden sm:block">
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-blue-600 border-b border-blue-700">
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Rumah Sakit</th>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Status</th>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Waktu Rujuk</th>
            ...
            <tbody class="divide-y divide-gray-300">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-4 text-center text-sm text-gray-600">
                        {{ ($rujukans->currentPage() - 1) * $rujukans->perPage() + $loop->iteration }}
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 bg-gradient-to-br from-purple-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-sm">
                                {{ strtoupper(substr($rujukan->korban->nama ?? '-', 0, 1)) }}
                            </div>
                            <div>
                                <p class="font-medium text-gray-800">{{ $rujukan->korban->nama ?? '-' }}</p>
                                <p class="text-sm text-gray-500">
                                    {{ $rujukan->korban->jenis_kelamin ?? '-' }}, {{ $rujukan->korban->umur ?? '-' }} th
                                </p>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <p class="text-sm text-gray-800">{{ $rujukan->rumahSakit->nama ?? '-' }}</p>
                        <p class="text-sm text-gray-500">{{ Str::limit($rujukan->rumahSakit->alamat ?? '-', 30) }}</p>
                    </td>
                    <td class="px-6 py-4">
                        @php
                        $statusClasses = [
                        'dirujuk' => 'bg-yellow-100 text-yellow-700',
                        'diterima' => 'bg-blue-100 text-blue-700',
                        'selesai' => 'bg-green-100 text-green-700',
                        ];
                        $statusClass = $statusClasses[$rujukan->status] ?? 'bg-gray-100 text-gray-700';
                        $statusLabels = [
                        'dirujuk' => 'Dirujuk',
                        'diterima' => 'Diterima',
                        'selesai' => 'Selesai',
                    ...
                    <td class="px-6 py-4">
                        <p class="text-sm text-gray-500">
                            {{ $rujukan->waktu_rujuk ? $rujukan->waktu_rujuk->locale('id')->translatedFormat('d M Y, H:i') : '-' }}
                        </p>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('admin.rujukan.show', $rujukan->id) }}"
                            ...
                            <form action="{{ route('admin.rujukan.destroy', $rujukan->id) }}" method="POST"
                                onsubmit="return confirm('Yakin ingin menghapus data rujukan ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                ...
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <p class="text-gray-500 font-medium">Tidak ada data rujukan</p>
                            <p class="text-gray-400 text-sm mt-1">Tambahkan data rujukan baru untuk memulai</p>
                        </div>
                    </td>
                </tr>
                @endforelse
